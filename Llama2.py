...
@st.cache_data
def Llama_70B(token,prompt, temp, top_p,seed):
...
def Display_Llama2(token):
    st.markdown("<h1 style=text-align:center;'>Meta Llama 70B</h1>", unsafe_allow_html=True)
    ...
    if prompt:
        st.session_state.messages_lama.append({"role": "user", "content": prompt})
        with st.chat_message("user"):
            st.write(prompt)
            
        result = Llama_70B(token,prompt, st.session_state.temperature_val, st.session_state.top_p_val,st.session_state.val2)

        st.session_state.messages_lama.append({"role": "assistant", "content": result})
